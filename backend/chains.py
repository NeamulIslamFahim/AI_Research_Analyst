...
RESEARCH_PROMPT = PromptTemplate(
    ...
    template=(
        ...
        "- score_relevance and score_quality MUST be integers from 0 to 10, reflecting how well the paper matches the user's question and its academic rigor.\n"
        "- If fulltext_available is true, summary_full_paper MUST be a deep, analytical summary of the full paper content.\n"
        "- If fulltext_available is false, summary_full_paper MUST be a concise, abstract-based summary that avoids stating that full text was missing.\n"
        "- summary_full_paper MUST be a cohesive paragraph of 3-5 sentences. It should connect the paper's context, the problem it addresses, the core approach, and the significance of its findings.\n"
        "- proposed_model_or_approach MUST be an analytical paragraph of 3-5 sentences focused on methodology. It should describe the method, its components, the evaluation setting, and why the approach was chosen, using cautious language when details are limited.\n"
        "- If the paper does not propose a new method, say so clearly using cautious language such as 'The source metadata does not expose a distinct new method'.\n"
